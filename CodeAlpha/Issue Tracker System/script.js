function submitIssue(e) {
    ...
    if ((description.length == 0) || (assignedTo.length == 0)) {
      ...
      const issue = { id, description, severity, assignedTo, status ,created};
      let issues = [];
  ...
  const editIssue = (id) => {
    // Retrieve issue details using id from localStorage or any other source
    const issues = JSON.parse(localStorage.getItem('issues'));
    const currentIssue = issues.find((issue) => issue.id == id);
  
    // Example: If you want to edit the description
    const newDescription = prompt('Enter the updated description:', currentIssue.description);
    if (newDescription !== null && newDescription !== '') {
      currentIssue.description = newDescription;
      localStorage.setItem('issues', JSON.stringify(issues));
      fetchIssues(); // Update the displayed issues
    }
  };
  ...
  const fetchIssues = () => {
    ...
    for (var i = 0; i < issues.length; i++) {
      const { id, description, severity, assignedTo, status, created } = issues[i];
    ...
    <button onclick="editIssue(${id})" class="btn btn-info edit-button">Edit</button>
    </div>`;
    }
  }
  fetchIssues();
